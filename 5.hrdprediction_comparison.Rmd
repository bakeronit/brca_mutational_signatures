...
df_hrd |> filter(hr_status_combined=="Inconsistent") |> left_join(hrdsum) |> left_join(result_df)|> select(sample_id, cohort,HRDetect,CHORD, `HRD-sum`, updated_germline_classification, A, B, class_A_mutations, class_B_mutations) |> gt() |> as_raw_html()
```


```{r hrd_comparison_fig1, fig.width=9, fig.height=9}
p1 <- df_hrd |> mutate(hr_status_combined=factor(hr_status_combined,levels=c("HRD","non-HRD","Inconsistent"))) |> 
  ...
  ggrepel::geom_label_repel(data=df_hrd |> filter(hr_status_combined=="Inconsistent"), aes(label=sample_id), size=3.6, color="black") + 
...
group_labeller <- c("additional_brca1"="VUS BRCA1","additional_brca2"="VUS BRCA2","additional"="VUS BRCA1/2", "exclude"="Exclude","negative_brca1/2"="Negative BRCA1/2", "positive_brca1"="Positive BRCA1", "positive_brca2"="Positive BRCA2")
