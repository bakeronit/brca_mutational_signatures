...
df_hrd |> filter(hr_status_combined=="Inconsistent") |> left_join(hrdsum) |> left_join(result_df)|> 
  select(sample_id, cohort,HRDetect,CHORD, `HRD-sum`, updated_germline_classification, B, D, class_B_mutations, class_D_mutations) |> 
...
pa<- 
  ...
  theme_Publication() + labs(x="Tumour samples",y="Score", title="HRDsum") + 
...
pb<- 
  ...
  theme_Publication() + labs(x="Tumour samples",y="Probability", title="HRDetect") + 
  theme(legend.position = "bottom", axis.text.x = element_blank(), axis.ticks.x = element_blank(), plot.title = element_text(face="plain", size=rel(0.8),hjust = 0.5, vjust=-4))
...
pc<- 
  ...
  theme_Publication() + labs(x="Tumour samples",y="Probability", title="CHORD") + 
  theme(legend.position = "bottom", axis.text.x = element_blank(), axis.ticks.x = element_blank(), plot.title = element_text(face="plain", size=rel(0.8),hjust = 0.5, vjust=-4))
...
pe <- plot_df |> 
  ...
  geom_label(data=plot_df |> filter(hr_status!="HRP" | updated_germline_classification %notin% c("positive_brca1","positive_brca2","additional")),
             aes(label=..count.., color=type, fill=updated_germline_classification),
             stat="count",position="fill",size=3.6,hjust=1.4,show.legend = FALSE) + scale_color_manual(values=c("white","black")) +
  ...
  geom_label(data=plot_df |> filter(hr_status=="HRP" & updated_germline_classification %in% c("additional")),
             aes(label=..count.., color=type, fill=updated_germline_classification),
             stat="count",position="fill",size=3.6, hjust=5.9,show.legend = FALSE) + 
  ...
  labs(x="",y="Proportion", fill="Individual classification") +
  theme_Publication() +
...
pe
#ggsave("figures/figure2e.rev.pdf", pe, height = 3.6, width = 12.2, dpi = 320)
