...
There are five samples had inconsistent HR status perdition from two methods.

```{r hrd_comparison_fig1, fig.width=6.2, fig.height=4.5}
df_hrd |> mutate(hr_status_combined=factor(hr_status_combined,levels=c("HRD","non-HRD","Inconsistent"))) |> 
...
Notably, four out of these five samples were excluded by germline classification, and `TCGA-A7-A13D` is classified as brca_negative which is consistent with CHORD result.
...
ggplot() + geom_bar(data=df_hrd |>  mutate(hr_status_combined=factor(hr_status_combined,levels=c("HRD","non-HRD","Inconsistent"))), aes(x=cohort, fill=hr_status_combined),stat = "count") + 
...
chord_brca12 |> filter(hr_status_combined=="HRD") |> 
