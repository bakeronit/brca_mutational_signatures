...
The HRDetect method uses the mutational signatures to predict BRCA1/2 deficiency as a surrogate for HR deficiency, including base substitution signatures 3 and 8, indel patterns, SVs and the copy number-based score HRD loss of heterozygosity (LOH). The `HRDetect_pipeline` function of [signature.tools.lib](https://github.com/Nik-Zainal-Group/signature.tools.lib) was used for the HRDetect implementation with 1000 bootstraps and “Breast” specific signatures. A score of >0.7 is considered to represent deficiency.
