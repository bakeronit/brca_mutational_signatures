...
df_hrd |> 
  ...
  labs(x="Tumour samples", y="HRD probability (HRDetect)") +
  theme_Publication() +
...
df_hrd |> 
  ...
  theme_Publication() +
  theme(axis.text.x = element_blank(), axis.ticks.x = element_blank()) + labs(x="Tumour samples", y="HRD probability (HRDetect)")
```
...
df_hrd |> 
  mutate(cohort=case_when(grepl("FBC",sample_id) ~ "Familial Breast",
                                     ...
                                     TRUE ~ "Q-IMPROvE"),
         cohort=factor(cohort, levels=c("Familial Breast","TCGA","MAGIC","Q-IMPROvE"))) |> 
  mutate(type=ifelse(Probability>=0.7, "HRD","non-HRD")) |> 
...
```{r, eval=FALSE}
unrepaired_correction <- read.table("../1.denovo_extraction/data/GRCh38/MAGIC.ffpe_unrepaired_corrected.matrix.txt", header = TRUE,check.names = FALSE)
rownames(unrepaired_correction) <- rownames(COSMIC30_subs_signatures)
...
ffpe_corr_sbs_df <- colnames(unrepaired_correction) |> set_names() |> map(\(x) Zainal_SBS_fit_from_matrix(x, unrepaired_correction, cosmicv2_signatures[sigsToUse])) |> list_rbind(names_to = "sample")
...
Comparing the HRD prediction results before and after FFPE correction, there is no impact to number of HRD samples except some minor value changes.

```{r hrdetect_fig4, fig.height=4, fig.width=5}
df_hrd_magic <- read_tsv("results/hrdetect_hrdprediction_355.tsv") |> filter(startsWith(sample,"MAGIC")) |> separate_wider_delim(sample,names = c("sample_id","tumour_id"), delim = ".") |> select(sample_id,tumour_id, Probability)

df_hrd_magic_corrected <- read_tsv("results/magic_ffpe_corrected_hrdetect.txt") |> select(tumour_id,Probability)

df_hrd_magic |> left_join(df_hrd_magic_corrected, by="tumour_id") |> rename("Original"=Probability.x,"FFPE-corrected"=Probability.y) |> 
  filter(!tumour_id %in% extra_tumours) |> 
  arrange(desc(Original)) |> mutate(sample_id=factor(sample_id, levels=sample_id)) |> 
  pivot_longer(c(Original, `FFPE-corrected`)) |> mutate(name=factor(name, levels=c("Original", "FFPE-corrected"))) |> 
  ggplot(aes(x=sample_id,y=value, color=name)) + geom_point(alpha=.4, size=1.6) +
  geom_hline(yintercept = 0.7, color="grey", lty=2, linewidth=1) +
  labs(x="Tumour samples",y="HRD Probability (HRDetect)", color="") + theme_Publication() +
