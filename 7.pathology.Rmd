...
We collected pathology information for samples including: Grade, ER, PR, HER2 etc and added this to supplementary table 4.
...
df |> head(10) |>  gt() |> as_raw_html()
```

## The correlation of pathology and HR status

We would like to exam that whether ER, TNBC status, grade, etc are all significantly correlated with HR status.
...
ggsave("p1.pdf", plot, width = 16, height = 6)
```
...
ggsave("p2.pdf", p, width = 16, height = 5)
plot/(p2+p3+p4 + plot_layout(widths = c(0.7,0.15,0.15))) + plot_layout(heights=c(0.65, 0.35)) + plot_annotation(tag_levels = "A")
```