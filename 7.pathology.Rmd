...
df2 |> select(c(sample_id, age_group, gender, ER, PR, HER2, tumor_size_category, grade_clean, morphology_clean)) |> rename("Grade"=grade_clean, "Morphology" = morphology_clean, "Tumor size category" = tumor_size_category, "Donor ID" = sample_id, "Age Group" = age_group, "Sex" = gender) |> 
  head(10) |>  gt() |> as_raw_html()

df2 |> select(sample_id, age_group, gender, grade_clean, ER, PR, HER2) |> 
  mutate(grade=case_when(grade_clean=="Grade I" ~ 1,
                               grade_clean=="Grade II" ~ 2,
                               grade_clean=="Grade III" ~ 3,
                               TRUE ~ NA),
         tnbc=case_when(ER=="Negative" & PR=="Negative" & HER2=="Negative" ~ "yes",
                        ER=="Positive" | PR=="Positive" | HER2=="Positive" ~ "no",
                        TRUE ~ "n/a")) |> 
  select(sample_id, age_group, gender, grade, ER, PR, HER2, tnbc) #|> 
  #write_tsv("results/pathology.tsv")
```
...
#ggsave("figures/supp-fig5-p1.pdf", plot, width = 16, height = 6)
```

```{r, warning=FALSE, fig.height=3, fig.width=7}
plot_df <- df2 |> mutate(tnbc=case_when(ER=="Negative" & PR=="Negative" & HER2=="Negative" ~ "yes",
                        ER=="Positive" | PR=="Positive" | HER2=="Positive" ~ "no",
                        TRUE ~ "n/a")) 
...
```{r supp-fig5, fig.width=16, fig.height=16}
...
plot/(p2+p3+p4 + plot_layout(widths = c(0.7,0.15,0.15))) + plot_layout(heights=c(0.65, 0.35)) + plot_annotation(tag_levels = "a") & theme(plot.tag = element_text(face="bold", size=20))

p <- (p2+p3+p4 + plot_layout(widths = c(0.7,0.15,0.15)))
#ggsave("figures/supp-fig5-p2.pdf", p, width = 16, height = 4.6)
```