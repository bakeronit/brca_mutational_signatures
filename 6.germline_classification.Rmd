...
num_of_variant <- read_tsv(paste0(data_dir, "germline_mutation_classification.txt")) |> count(Class_updated) |> pull(n)
...
We merged with this classification with thier tumour HRD profiles and generate a big table [Supplementary Table 4](https://docs.google.com/spreadsheets/d/1rgva1pfGp3ScCln8a0WScAAEoazS1-dUu5NfeF_1AD0/edit?gid=1859159045#gid=1859159045): 
...
sv_df |> mutate(sample_id=ifelse(startsWith(sample_id,"MAGIC"),magic_id_conversion[sample_id],sample_id)) |> gt() |> as_raw_html()
...
read_tsv("results/brca_germline_clf_tumor_hrd.add_sv.add_somatic.tsv") |> 
  mutate(sample_id=ifelse(cohort=="MAGIC",magic_id_conversion[sample_id],sample_id)) |> 
  select(-tumour_id) |> 
  write_tsv("results/supplementary-table4.tsv")
```
...
```{r supplementary_fig2,fig.height=8, fig.width=15.6, warning=FALSE, eval=FALSE}
...
#ggsave("figures/Figure1b.pdf", height = 9, width = 8.6, dpi = 320)
```
