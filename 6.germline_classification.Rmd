...
```{r, Figure2, fig.height=8, fig.width=8.6, warning=FALSE, fig.cap="This figure illustrates the method we classfied each individual as different germline group"}
p_tmp <- tree_df |> 
...
group_labeller <- c("additional_brca1"="Additional BRCA1","additional_brca2"="Additional BRCA2", "exclude"="Exclude","negative_brca1/2"="Negative BRCA1/2", "positive_brca1"="Positive BRCA1", "positive_brca2"="Positive BRCA2")
...
**The number of individuals labelled as different groups for 4 cohorts**

```{r germline_classification_cohort, fig.width=7.6, fig.height=3.2}
group_color_mapping <- c("additional_brca1"="#0099B4", "additional_brca2"="#FDAF91" , "exclude" = "#1B1919", "negative_brca1/2"="#ADB6B6", "positive_brca1" = "#00468B", "positive_brca2"="#ED0000")

result_df |>
  count(cohort,updated_germline_classification) |> 
  complete(cohort,updated_germline_classification) |> 
  mutate(n = replace_na(n, 0), cohort=factor(cohort, levels=cohort_levels)) |> 
  mutate(updated_germline_classification = factor(updated_germline_classification, levels=c("positive_brca1","positive_brca2","additional_brca1", "additional_brca2","exclude","negative_brca1/2"))) |> 
  ggplot(aes(x = cohort, y = n, fill = updated_germline_classification)) +
  geom_bar(stat = "identity", position = "dodge", width = 0.7) +
  geom_text(aes(label = n),position = position_dodge(width = 0.7),vjust = -0.5, size=2.5) +
  theme_Publication() +
  scale_fill_manual(labels = group_labeller, values = group_color_mapping) + 
  labs(x = "", y = "Count", fill = "Germline classification") +
