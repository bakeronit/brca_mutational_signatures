...
p_chord_threshold<- ggplot(df_hrd, aes(x=sample_id, y=p_hrd)) + geom_point(alpha=.6, color="#3B4992") + 
  ...
  theme_Publication() + labs(x="Tumour samples",y=stringr::str_wrap("HRD probability (CHORD)", 15)) + 
...
exclude_plp_df <- result_df |> select(cohort, sample_id, hrd_type, updated_germline_classification, A, class_A_mutations, pathSV) |> filter(A>0,hrd_type!="none") |> 
...
additional_df <- 
  result_df |> select(cohort, sample_id, hrd_type, updated_germline_classification, E, class_E_mutations, pathSV) |> filter(grepl("additional", updated_germline_classification), hrd_type!="none") |> 
  mutate(variant=class_E_mutations |> map_chr(transform_variant)) |> 
  ...
  mutate(col=case_when(row==7~32.6, hrd_type=="BRCA1_type"~25.5, hrd_type=="BRCA2_type"~17), row=ifelse(row==7, 6, row), row=ifelse(hrd_type=="BRCA2_type", row+2, row))

result_df |> select(cohort, sample_id, hrd_type, updated_germline_classification, A, class_A_mutations, class_E_mutations,E,pathSV) |> filter(A>0 | E>0) |> 
  filter(hrd_type!="none", startsWith(updated_germline_classification,"additional")) |> 
  mutate(variant=str_replace_all(class_E_mutations, "c\\.(.*)\\((BRCA\\d+)\\)", "\\2 (\\1)")) |> 
