...
ggplot(mosaic_df) + geom_mosaic(aes(x=product(hr_status,updated_germline_classification), fill=hr_status)) + 
  ...
  labs(x="", y="", fill="HR status") + 
   scale_y_productlist(
    labels = c("HRD","non-HRD"),
    expand = c(0, 0)
  ) +
   scale_x_productlist(
    labels = c("Negative BRCA1/2",stringr::str_wrap("Positive BRCA1", 6),stringr::str_wrap("Positive BRCA2", 6)),
    expand = c(0, 0.01)
  ) +
...
pb <- mosaic_df |> 
  ...
  scale_fill_manual(values=c(my_blues[1:2], "#EFC000"), labels=c("BRCA1","BRCA2","non-HRD")) +
  labs(x="", y="", fill="HR status") + 
  scale_y_productlist(
    labels = c("HRD (BRCA1)", "HRD (BRCA2)","non-HRD"),
    expand = c(0, 0)
  ) +
   scale_x_productlist(
    labels = c("Negative BRCA1/2",stringr::str_wrap("Positive BRCA1", 6),stringr::str_wrap("Positive BRCA2", 6)),
...
29 had been excluded due to carrying P/LP or suspicious VUS in non-BRCA1/2 genes, out of these 29 individuals, 4 of them are HRD (BRCA1), 4 are HRD (BRCA2) and 2 of them are HRD (Undetermined). 
...
hrdtype_labeller <- c(stringr::str_wrap("HRD (BRCA1)",8),stringr::str_wrap("HRD (BRCA2)",8),stringr::str_wrap("HRD (Undetermined)",10), "non-HRD")
...
pc<-result_df |> select(cohort,sample_id, hrd_type, updated_germline_classification, A, B) |> 
  ...
  theme( legend.position = "inside", 
        legend.position.inside  = c(0.88,0.66), 
