...
```{r chord_fig4, fig.height=4.5, fig.width=4.8}
df_tmp <- df_hrd |> group_by(cohort) |> mutate(total=n()) |> add_count(hr_status) |> filter(hr_status=="HRD") 

ggplot() + geom_bar(data=df_hrd, aes(x=cohort, fill=hr_status),stat = "count") + 
...
```{r chord_fig5, warning=FALSE, fig.width=4.8, fig.height=4.5}
...
corrected_mutation <- read_tsv("/mnt/backedup/home/jiaZ/working/bioprojects/breast_cancer_tms_Amanda2023/1.denovo_extraction/data/GRCh38/MAGIC.ffpe_unrepaired_corrected.extractor_sorted.matrix.txt")
...
```{r chord_fig6, fig.height=4, fig.width=5}
