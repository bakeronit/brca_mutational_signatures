...
load("scripts/hrdsum_chrominfo.rda")
...
run_hrdsum <- function(cnv_file) {
  ...
  mutate(cn_B=total.copy.number.inTumour-minor.copy.number.inTumour, Chromosome=paste0("chr",Chromosome)) |> add_column(ploidy=0, sample="FBC013587") |> 
select(sample, everything()) |> scar_score() |> as.data.frame()
}
...
df_hrdsum <- read_tsv("results/chord_hrdprediction_355.tsv") |>
  ...
  mutate(sample_id=factor(sample_id,levels=sample_id), hr_status=ifelse(hr_status=="HR_deficient","HRD", "non-HRD"), cohort=factor(cohort, levels=cohort_levels))
