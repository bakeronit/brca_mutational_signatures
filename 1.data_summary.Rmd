...
df_magic <- read_csv("~/working/bioprojects/breast_cancer_tms_Amanda2023/0.data_resource/GRCh38/Waddell_-_Magic_WGS_StudySomaticReport.csv.139") |> filter(!is.na(ascatAnalysis)) |> rename("sample_id"=donorSubmittedWithID, "tumour_id"=testSampleLabel) |> filter(!tumour_id %in% extra_tumours)
...
df_all |> 
  group_by(cohort) |> 
  summarise(mean_purity = mean(ascatPurityScore), min=min(ascatPurityScore), max=max(ascatPurityScore)) |> gt()
```
