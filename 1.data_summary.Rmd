...
df_magic <- read_csv(paste0(data_dir, "Waddell_-_Magic_WGS_StudySomaticReport.csv.139")) |> filter(!is.na(ascatAnalysis)) |> rename("sample_id"=donorSubmittedWithID, "tumour_id"=testSampleLabel) |> filter(!tumour_id %in% extra_tumours)
...
google_sheet_supp1 <- sv_df |> group_by(sample_id) |> summarise(total_sv=n()) |> ungroup() |> 
  ...
  arrange(Study) |> 
  rename("Sample ID" = sample_id,"SV" = total_sv, "Tumour depth" = t_depth, "Normal depth" = n_depth, "Tumour purity" = ascatPurityScore, "TMB" = TMB_nonSyn, "SNV" = snv, "Indel" = indel, "SV" = total_sv) 

google_sheet_supp1 |> head(5) |> gt() |> as_raw_html()
