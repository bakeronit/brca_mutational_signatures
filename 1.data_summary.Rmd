...
All samples were processed with a uniform pipeline and wWe applied the dual calling pipeline for analysing WGS data of tumour samples with matched germline samples to identify somatic SNV and indel changes. Specifically, we aligned the short-read sequencing reads to the human genome assembly (GRCh38) using BWA, and high confident somatic SNV were identified as the intersect of post-filtered output of qSNP (cite) and GATK HaplotypeCaller (cite). Short insertion and deletions (1-50bp) were called with the GATK HaplotyperCaller. Mutations were annotated with qannotate and SnpEff to identify somatic-specific mutations and filter mutations located within 6 base pairs of a homopolymer region.


```{r, message=FALSE}
# load grafli report
df_none2019 <- read_csv("~/working/bioprojects/breast_cancer_tms_Amanda2023/0.data_resource/GRCh38/Simpson_-_FamilialBreast_SomaticReport_GRCh38_77.csv") |> filter(!is.na(ascatAnalysis)) |> 
  rename("sample_id"=donorPublicationID)

uuid_tsv <- read_tsv("~/working/bioprojects/breast_cancer_tms_Amanda2023/0.data_resource/GRCh38/uuid_case.txt",col_names = c("uuid","sample_id"))
df_tcga <- read_csv("~/working/bioprojects/breast_cancer_tms_Amanda2023/0.data_resource/GRCh38/TCGA_-_breast_cancer_SomaticReport_GRCh38.csv") |> filter(!is.na(ascatAnalysis)) |> 
  left_join(uuid_tsv, by=c("donorSubmittedWithID"="uuid")) 

df_magic <- read_csv("~/working/bioprojects/breast_cancer_tms_Amanda2023/0.data_resource/GRCh38/Waddell_-_Magic_WGS_StudySomaticReport.csv.139") |> filter(!is.na(ascatAnalysis)) |> rename("sample_id"=donorSubmittedWithID, "tumour_id"=testSampleLabel)
...
df_qimprove <- read_tsv("~/working/bioprojects/breast_cancer_tms_Amanda2023/0.data_resource/GRCh38/GenomiQa_-_qimprove_Somatic_Report_GRCh38_for_jia_no_recurrance.tsv") |> 
  filter(! controlSampleIRI=="collectedsample:4a6e39fd-4e92-496d-a1ce-98b1a9340d8c") |> add_column(studyLabel="qimprove") |> rename("sample_id"=patientLabel, "tumour_id"=testSampleLabel)
```

After removing samples with less then 20x depth in tumour sample, we only keep the primary tumour from each patient.

```{r, fig.width=9.2, fig.height=4}
extra_tumours <- c("QIPAH006_20230927_0012", "QIPAH011_20230927_0017", "QIPAH013_20230927_0029", "QIPAH002_20230927_0023","2020_118T2")
depth_df <- rbind(read_tsv("../0.data_resource/GRCh38/sequencing_depth/nones.depth.txt", col_names = c("sample","t_depth","n_depth")) |> add_column(cohort="familial"),
                  read_tsv("../0.data_resource/GRCh38/sequencing_depth/tcga.depth.txt", col_names = c("sample","t_depth","n_depth")) |> add_column(cohort="tcga"),
                  read_tsv("../0.data_resource/GRCh38/sequencing_depth/magic.depth.txt", col_names = c("sample","t_depth","n_depth")) |> add_column(cohort="magic"),
                  read_tsv("../0.data_resource/GRCh38/sequencing_depth/qimprove.depth.txt",col_names = c("sample","t_depth","n_depth")) |> add_column(cohort="qimprove")
                  ) |> 
...
#ggsave("figures/supplementary_fig1.png",width = 9,height = 4)
```

A figure showing the tumour purity scores, TMB in cohorts.

```{r, fig.width=9.6, fig.height=5.5, warning=FALSE, message=FALSE}
df_all <- rbind(df_none2019 |> select(studyLabel,sample_id,ascatPurityScore,TMB_nonSyn),
  ...
  mutate(sample_id = str_split_i(sample_id,"\\.",i=1))
...
df_all |> 
  group_by(cohort) |> 
  summarise(mean_purity = mean(ascatPurityScore), min=min(ascatPurityScore), max=max(ascatPurityScore)) |> knitr::kable()

#ggsave("figures/supplementary_fig2.png", width = 9.6, height = 4.5)
```
...
```{r, fig.width=9.2, fig.height=4.6, message=FALSE, warning=FALSE, results='hide'}
summarise_vcfs <- function(pdir, sample) {
  ...
  df <- data.frame(snv=n_snv,indel=n_indel)
  df
}
...
#ggsave("figures/supplementary_fig3.png",dpi = 300,width = 9.2,height = 4.6)

```

```{r, fig.width=9.2, fig.height=3.6, warning=FALSE}
...
#ggsave("figures/supplementary_fig4.png",dpi = 300,width = 9.2,height = 3.6)
```