...
- [Familial breast cancer](https://pubmed.ncbi.nlm.nih.gov/31090900/) (N=77)
...
google_sheet_supp1 |> 
  gt() |> as_raw_html()
google_sheet_supp1 |> write_tsv("results/supplementary-table1.tsv")
```
...
familial <- familial |> rename("age"=`Age at diagnosis (years)`, "sample_id"=Donor, "gender"=Gender, "morph"=`Morphology coding`, "stage"=Grade, "size"=`Tumour Size (mm)`) |> mutate(age=as.integer(age)) |> 
  select(sample_id, age, gender, morph, size, stage, ER, PR, HER2)
```
...
magic <- magic |> rename("age"=`Age at diagnosis`, "sample_id"=`MAGIC ID`, "gender"=Gender, "morph"=Morphology, "stage"=Grade, "size"=`Tumour size(mm)`) |> mutate(age=as.integer(age)) |> 
  select(sample_id, age, gender, morph, size, stage, ER, PR, HER2)
```
...
tcga <- read_tsv("data/TCGA-BRCA_116.txt") |> select(bcr_patient_barcode, gender,age_at_diagnosis,er_status_by_ihc, pr_status_by_ihc,her2_status_by_ihc,icd_o_3_histology, ajcc_pathologic_tumor_stage, ajcc_tumor_pathologic_pt) %>% 
  rename("sample_id"=bcr_patient_barcode, "age"=age_at_diagnosis,"ER"=er_status_by_ihc, "PR"= pr_status_by_ihc, "HER2"=her2_status_by_ihc,"morph"=icd_o_3_histology,"stage"= ajcc_pathologic_tumor_stage, "size"=ajcc_tumor_pathologic_pt) |> 
  select(sample_id, age, gender, morph, size, stage, ER, PR, HER2)

tcga$HER2 <- tcga$HER2 |>  str_replace("\\[Not Evaluated\\]","n/a")
...
tcga <- tcga |> filter(sample_id %in% valid_samples)
```

Different studies have their own framework to record tumour histopath information, need to consolidate them.

```{r, warning=FALSE, echo=TRUE}
df <- rbind(familial, magic, tcga) |> replace_na(list(stage="n/a", size="n/a", gender="n/a", morph="n/a",age="n/a",ER="n/a",PR="n/a", HER2="n/a"))
stage_tags <-  df$stage |> unique() 
stage_clean<- c("III", "II","n/a", "I", "III", "II", "I", "II/III", "II","III", "I","II", "I", "III","III", "X")
names(stage_clean) <- stage_tags

morph_tags <- df$morph |> unique()
morph_clean <-c("Invasive Carcinoma of No Special Type",
                      ...
                      "Lobular Carcinoma",
                      "Mucinous Carcinoma",
                      "HGDCIS with microinvasion",
                      "Mixed IDC and Lobular Carcinoma",
                      "Other",
                      "Mixed IDC and Lobular Carcinoma",
                      "Invasive Ductal Carcinoma, mixed",
                      "Metaplastic Carcinoma",
                      "Mucinous Carcinoma",
                      "Mucinous Carcinoma",
                      "Apocrine Carcinoma",
                      ...
                      "Medullary Carcinoma")
names(morph_clean) <- morph_tags

df |> mutate(gender=ifelse(toupper(gender) %in% c("MALE","M"), "Male", ifelse(toupper(gender) %in% c("FEMALE","F"), "Female", "n/a"))) |> 
  mutate(tcode=case_when(startsWith(size,"T") ~ size, 
                         as.integer(size)>1 & as.integer(size)<=5 ~ "T1a", 
                         as.integer(size)>5 & as.integer(size)<=10 ~ "T1b", 
                         as.integer(size)>10 & as.integer(size) <=20 ~ "T1c", 
                         as.integer(size)>20 & as.integer(size) <=50 ~ "T2",
                         as.integer(size)>50 ~ "T3")) |> 
  mutate(stage=stage_clean[stage], morphology=morph_clean[morph]) |> head(n=10) |>  gt() |> as_raw_html()
```
