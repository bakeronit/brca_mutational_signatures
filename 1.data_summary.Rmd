...
familial <- familial |> rename("age"=`Age at diagnosis (years)`, "sample_id"=Donor, "gender"=Gender, "morph"=`Morphology coding`, "stage"=Grade, "tumor_size"=`Tumour Size (mm)`) |> mutate(age=as.integer(age)) |> 
  select(sample_id, age, gender, morph, tumor_size, stage, ER, PR, HER2)
```
...
magic <- magic |> rename("age"=`Age at diagnosis`, "sample_id"=`MAGIC ID`, "gender"=Gender, "morph"=Morphology, "stage"=Grade, "tumor_size"=`Tumour size(mm)`) |> mutate(age=as.integer(age)) |> 
  select(sample_id, age, gender, morph, tumor_size, stage, ER, PR, HER2)
```
...
tcga <- read_tsv("data/TCGA-BRCA_116.txt") |> select(bcr_patient_barcode, gender,age_at_diagnosis,er_status_by_ihc, pr_status_by_ihc,her2_status_by_ihc,icd_o_3_histology, ajcc_pathologic_tumor_stage, ajcc_tumor_pathologic_pt) %>% 
  rename("sample_id"=bcr_patient_barcode, "age"=age_at_diagnosis,"ER"=er_status_by_ihc, "PR"= pr_status_by_ihc, "HER2"=her2_status_by_ihc,"morph"=icd_o_3_histology,"stage"= ajcc_pathologic_tumor_stage, "tumor_size"=ajcc_tumor_pathologic_pt) |> 
  select(sample_id, age, gender, morph, tumor_size, stage, ER, PR, HER2)
...
qimprove <- qimprove |> rename("age"=`Age at diagnosis`, "sample_id"=`GenomiQa id`, "gender"=Gender, "morph"=Morphology, "stage"=Grade, "tumor_size"=`Tumour size (mm; clinical or imaging)`) |> mutate(age=as.integer(age)) |> 
  select(sample_id, age, gender, morph, tumor_size, stage, ER, PR, HER2)
```
...
df <- rbind(familial, magic, tcga, qimprove) |> replace_na(list(stage="n/a", size="n/a", gender="n/a", morph="n/a",age="n/a",ER="n/a",PR="n/a", HER2="n/a"))
...
df |> mutate(gender=ifelse(toupper(gender) %in% c("MALE","M"), "Male", ifelse(toupper(gender) %in% c("FEMALE","F"), "Female", "n/a"))) |> 
  mutate(tumor_size_category=case_when(startsWith(tumor_size,"T") ~ tumor_size, 
                         ...
                         TRUE ~ "n/a"),
         stage_clean=stage_map[stage], 
         morphology_clean=morphology_map[morph],
         sample_id=ifelse(startsWith(sample_id,"MAGIC"), magic_id_conversion[sample_id], sample_id),
         morphology_clean=ifelse(morphology_clean=="Ductal_Carcinoma_In_Situ" & stage=="III", "High_Grade_DCIS", morphology_clean))|>  gt() |> as_raw_html()
```
